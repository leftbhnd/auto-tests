...
def test_no_connection_modal(clickOn, typeText, screenDiffChecker):
    ...
    assert screenDiffChecker(
        'localization/az_AZ/no_connection_modal.png'
    ) is None
...
def test_radius_modal(clickOn, screenDiffChecker):
    ...
    time.sleep(slowly)
    assert screenDiffChecker(
        'localization/az_AZ/radius_modal.png'
    ) is None
...
def test_answer_log(clickOn, node, screenDiffChecker):
    ...
    time.sleep(slowly)
    assert screenDiffChecker(
        'localization/az_AZ/test_answers_log.png',
        (0, 40, 1280, 660)
    ) is None
...
def test_speech_settings(clickOn, screenDiffChecker):
    ...
    assert screenDiffChecker(
        'localization/az_AZ/speech_settings.png',
        (0, 40, 1280, 660)
    ) is None
...
def test_testing_script(openPasswordModal, clickOn, typeText, screenDiffChecker):
    ...
    assert screenDiffChecker(
        'localization/az_AZ/script_is_running.png'
    ) is None
...
def test_main_camera(clickOn, screenDiffChecker):
    ...
    assert screenDiffChecker(
        'localization/az_AZ/testing_main_camera_header.png',
        (0, 40, 1280, 60)
    ) is None
...
def test_face_recognize(clickOn, screenDiffChecker):
    ...
    clickOn(btn.test_fr)
    assert screenDiffChecker(
        'localization/az_AZ/testing_face_recognize_header.png',
        (0, 40, 1280, 60)
    ) is None
...
def test_bottom_camera(clickOn, screenDiffChecker):
    ...
    assert screenDiffChecker(
        'localization/az_AZ/testing_bottom_camera_header.png',
        (0, 40, 1280, 60)
    ) is None
...
def test_fisheye_camera(clickOn, screenDiffChecker):
    ...
    assert screenDiffChecker(
        'localization/az_AZ/testing_fisheye_camera_header.png',
        (0, 40, 1280, 60)
    ) is None
...
def test_periphery_statuses(clickOn, screenDiffChecker):
    ...
    assert screenDiffChecker(
        'localization/az_AZ/periphery_statuses_modal.png'
    ) is None
...
def test_record_sound_start(clickOn, screenDiffChecker):
    ...
    assert screenDiffChecker(
        'localization/az_AZ/testing_record_sound_start.png'
    ) is None
...
def test_record_sound_finish(clickOn, screenDiffChecker):
    time.sleep(10)
    assert screenDiffChecker(
        'localization/az_AZ/testing_record_sound_finish.png'
    ) is None
...
def test_auto_mode_popup(clickOn, typeText, screenDiffChecker):
    ...
    clickOn(btn.back)
    assert screenDiffChecker(
        'localization/az_AZ/automode_popup.png'
    ) is None
...
def test_joy_mode_popup(clickOn, typeText, screenDiffChecker):
    ...
    clickOn(btn.back)
    assert screenDiffChecker(
        'localization/az_AZ/joy_mode_popup.png'
    ) is None
