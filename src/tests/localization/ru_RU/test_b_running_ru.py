...
def test_no_connection_modal(clickOn, typeText, screenDiffChecker):
    ...
    assert screenDiffChecker(
        'localization/ru_RU/run_no_connection_modal.png'
    ) is None
...
def test_radius_modal(clickOn, screenDiffChecker):
    ...
    time.sleep(slowly)
    assert screenDiffChecker(
        'localization/ru_RU/run_radius_modal.png'
    ) is None
...
def test_answer_log(clickOn, node, screenDiffChecker):
    ...
    time.sleep(slowly)
    assert screenDiffChecker(
        'localization/ru_RU/run_test_answers_log.png',
        (0, 40, 1280, 660)
    ) is None
...
def test_speech_settings(clickOn, screenDiffChecker):
    ...
    assert screenDiffChecker(
        'localization/ru_RU/run_speech_settings.png',
        (0, 40, 1280, 660)
    ) is None
...
def test_testing_script(clickOn, openServiceMenu, screenDiffChecker):
    ...
    time.sleep(default)
    assert screenDiffChecker(
        'localization/ru_RU/run_script_is_running.png'
    ) is None
...
def test_main_camera(clickOn, screenDiffChecker):
    ...
    assert screenDiffChecker(
        'localization/ru_RU/run_testing_main_camera_header.png',
        (0, 40, 1280, 60)
    ) is None
...
def test_face_recognize(clickOn, screenDiffChecker):
    ...
    clickOn(btn.test_fr)
    assert screenDiffChecker(
        'localization/ru_RU/run_testing_face_recognize_header.png',
        (0, 40, 1280, 60)
    ) is None
...
def test_bottom_camera(clickOn, screenDiffChecker):
    ...
    assert screenDiffChecker(
        'localization/ru_RU/run_testing_bottom_camera_header.png',
        (0, 40, 1280, 60)
    ) is None
...
def test_fisheye_camera(clickOn, screenDiffChecker):
    ...
    assert screenDiffChecker(
        'localization/ru_RU/run_testing_fisheye_camera_header.png',
        (0, 40, 1280, 60)
    ) is None
...
def test_periphery_statuses(clickOn, screenDiffChecker):
    ...
    assert screenDiffChecker(
        'localization/ru_RU/run_periphery_statuses_modal.png'
    ) is None
...
def test_record_sound_start(clickOn, screenDiffChecker):
    ...
    assert screenDiffChecker(
        'localization/ru_RU/run_testing_record_sound_start.png'
    ) is None
...
def test_auto_mode_popup(clickOn, typeText, screenDiffChecker):
    ...
    clickOn(btn.back)
    assert screenDiffChecker(
        'localization/ru_RU/run_automode_popup.png'
    ) is None
...
def test_joy_mode_popup(clickOn, typeText, screenDiffChecker):
    ...
    clickOn(btn.back)
    assert screenDiffChecker(
        'localization/ru_RU/run_joy_mode_popup.png'
    ) is None
