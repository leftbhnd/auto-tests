...
def test_no_connection_modal(clickOn, typeText, screenDiffChecker):
    ...
    assert screenDiffChecker(
        'localization/fi_FI/no_connection_modal.png'
    ) is None
...
def test_radius_modal(clickOn, screenDiffChecker):
    ...
    time.sleep(slowly)
    assert screenDiffChecker(
        'localization/fi_FI/radius_modal.png'
    ) is None
...
def test_answer_log(clickOn, node, screenDiffChecker):
    ...
    time.sleep(slowly)
    assert screenDiffChecker(
        'localization/fi_FI/test_answers_log.png',
        (0, 40, 1280, 660)
    ) is None
...
def test_speech_settings(clickOn, screenDiffChecker):
    ...
    assert screenDiffChecker(
        'localization/fi_FI/speech_settings.png',
        (0, 40, 1280, 660)
    ) is None
...
def test_testing_script(openPasswordModal, clickOn, typeText, screenDiffChecker):
    ...
    assert screenDiffChecker(
        'localization/fi_FI/script_is_running.png'
    ) is None
...
def test_periphery_statuses(clickOn, screenDiffChecker):
    ...
    assert screenDiffChecker(
        'localization/fi_FI/periphery_statuses_modal.png'
    ) is None
...
def test_record_sound_finish(clickOn, screenDiffChecker):
    time.sleep(10)
    assert screenDiffChecker(
        'localization/fi_FI/testing_record_sound_finish.png'
    ) is None
...
def test_auto_mode_popup(clickOn, typeText, screenDiffChecker):
    ...
    clickOn(btn.back)
    assert screenDiffChecker(
        'localization/fi_FI/automode_popup.png'
    ) is None
...
def test_joy_mode_popup(clickOn, typeText, screenDiffChecker):
    ...
    clickOn(btn.back)
    assert screenDiffChecker(
        'localization/fi_FI/joy_mode_popup.png'
    ) is None
