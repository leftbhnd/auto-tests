...
def test_no_connection_modal(clickOn, typeText, screenDiffChecker):
    ...
    assert screenDiffChecker(
        'localization/uk_UA/no_connection_modal.png'
    ) is None
...
def test_radius_modal(clickOn, screenDiffChecker):
    ...
    time.sleep(slowly)
    assert screenDiffChecker(
        'localization/uk_UA/radius_modal.png'
    ) is None
...
def test_answer_log(clickOn, node, screenDiffChecker):
    ...
    asr_msg = AsrTtsMsg('тестове правило з лопатою')
    node.asrPub(asr_msg)
    time.sleep(slowly)
    assert screenDiffChecker(
        'localization/uk_UA/test_answers_log.png',
        (0, 40, 1280, 660)
    ) is None
...
def test_speech_settings(clickOn, screenDiffChecker):
    ...
    assert screenDiffChecker(
        'localization/uk_UA/speech_settings.png',
        (0, 40, 1280, 660)
    ) is None
...
def test_testing_script(openPasswordModal, clickOn, typeText, screenDiffChecker):
    ...
    assert screenDiffChecker(
        'localization/uk_UA/script_is_running.png'
    ) is None
...
def test_periphery_statuses(clickOn, screenDiffChecker):
    ...
    assert screenDiffChecker(
        'localization/uk_UA/periphery_statuses_modal.png'
    ) is None
...
def test_record_sound_start(clickOn, screenDiffChecker):
    ...
    assert screenDiffChecker(
        'localization/uk_UA/testing_record_sound_start.png'
    ) is None
...
def test_record_sound_finish(clickOn, screenDiffChecker):
    time.sleep(10)
    assert screenDiffChecker(
        'localization/uk_UA/testing_record_sound_finish.png'
    ) is None
...
def test_auto_mode_popup(clickOn, typeText, screenDiffChecker):
    ...
    clickOn(btn.back)
    assert screenDiffChecker(
        'localization/uk_UA/automode_popup.png'
    ) is None
...
def test_joy_mode_popup(clickOn, typeText, screenDiffChecker):
    ...
    clickOn(btn.back)
    assert screenDiffChecker(
        'localization/uk_UA/joy_mode_popup.png'
    ) is None
